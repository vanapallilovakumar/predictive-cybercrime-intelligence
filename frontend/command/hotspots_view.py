...
def render() -> None:
    ...
    st.markdown(
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 1.25rem;">
            ...
            <div style="font-family: 'Electrolize', sans-serif; font-size: 0.72rem; color: #ffffff; font-weight: 800; background: #000000; border: 1px solid #ffffff; padding: 0.35rem 0.75rem; border-radius: 2px;">
    ...
    for cell in hotspots:
        ...
        popup_content = (
            f"<div style='font-family: Electrolize, sans-serif; font-size: 12px; background: #000000; color: #ffffff; border: 1px solid #ffffff; padding: 8px; min-width: 170px;'>"
            f"<b style='font-size: 13px;'>{cell.get('district')} District</b><br>"
            f"<span style='color: #ffffff; font-weight: 900;'>Risk: {prob}% ({cell.get('risk_tier')})</span><br>"
    ...
    st.markdown(
        '<div style="font-size: 0.82rem; font-weight: 700; color: #a3a3a3; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem;">■ Spatial Cell Telemetry Table</div>',
        unsafe_allow_html=True,
    )
